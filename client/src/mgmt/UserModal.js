...
function UserModal(props) {
    ...
    const exclamation = (
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-exclamation-triangle text-danger" viewBox="0 0 16 16">
    ...
    return (
        <Modal size="md" onHide={props.hide} show={props.show} className="text-right">
            <div className="modal-content p-1" style={{ backgroundColor: "#f4f5f7" }}>
                <div className="bg-white h-100 rounded-lg border pb-4">
                    ...
                    <div className="mt-n2 text-center">
                        <input type="file" className="d-none" id="imageFile" onChange={handleImageChange} />
                        <div className="mx-auto" style={{ width: "30%" }}>
                            <label className="pointer my-0 text-muted" htmlFor="imageFile">
                                <img src={photo ? photo : "/unknownBig.png"} width="100%" className="mx-auto d-block my-0 rounded-circle"></img>
                            </label>
                        </div>
                        <h3 className="my-2" style={{ color: "#4b4b4ceb" }}>{props.user.name}</h3>
                    ...
                    <Form.Group className="my-4 mx-3">
                        ...
                        <Form.Label className="fontSmall mt-3 text-muted" hidden={field !== "photo"}>
                        ...
                        >
                            {
                                props.roles.map((userRole, index) => {
                                    return <option key={index} value={userRole}>{userRole}</option>
                                })
                            }
                        </Form.Control>
                    </Form.Group>
                    <div className="text-center">
                        ...
                        <span className={"my-2 text-success " + (result !== "success" ? "d-none" : "d-block")}
                        >הפרטים עודכנו בהצלחה</span>

                        <span className={"my-2 text-danger " + (result !== "error" ? "d-none" : "d-block")}
                        >הפעולה נכשלה, אנא וודא שהפרטים נכונים</span>

                        <span className={"my-2 text-danger " + (result !== "fileError" ? "d-none" : "d-block")}
                        >הקובץ גדול מידי או לא תקין</span>
                    </div>
                </div>
            </div>
        </Modal>
