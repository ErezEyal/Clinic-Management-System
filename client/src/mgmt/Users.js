...
function Users(props) {
    ...
    const USER_PHOTO_URL = "http://localhost:3000/api/user-photo";
    ...
    const removeIcon = (
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#dc3545d6" className="bi bi-x-circle-fill" viewBox="0 0 16 16">
            <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293 5.354 4.646z" />
        </svg>
    )

    const editIcon = (
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-pencil-fill text-secondary" viewBox="0 0 16 16">
            <path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708l-3-3zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207l6.5-6.5zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.499.499 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11l.178-.178z" />
        </svg>
    )

    const numOfPages = () => {
        if (filteredUsers.length % 10 === 0)
            return filteredUsers.length / 10 - 1;
        else
            return Math.floor(filteredUsers.length / 10);
    }
    ...
    const pagination = (
        <div>
            <div className="d-block text-center mt-3">
                ...
                <Button variant="link" className="d-inline text-decoration-none shadow-none"
                    onClick={() => setUsersPage(usersPage + 1)}
            ...
            <div className="d-block text-center mt-2">
                <span className="text-secondary smallFont">{filteredUsers.length} משתמשים</span>
            </div>
        </div>
    )
    ...
    const fetchUsers = () => {
        ...
        props.user.getIdToken(true).then(idToken => {
            props.postRequestWithToken(USERS_URL, idToken)
                .then(users => {
            ...
            return (
                ...
                >
                    <td className="pr-lg-4">
                        <img src={user.photo ? user.photo : "/unknownBig.png"} width="20px" className="d-inline mx-2 rounded-circle"></img>
                        <span>{user.name}</span>
                    </td>
                    <td>
                    ...
                    <td>
                        ...
                        <div className="d-inline-block float-left">
                            <span className="pointer mx-2" onClick={() => showUserModal(user)}>{editIcon}</span>
                            <span className="pointer mx-2" onClick={() => deleteUser(user)}>{removeIcon}</span>
                        </div>
                    </td>
                </tr>
            )
        ...
        const table = [
            <thead key="1">
                <tr style={{ backgroundColor: "#F5F8FA" }}>
                    ...
                    <th className="border font-weight-normal">כתובת מייל</th>
                    <th className="border font-weight-normal">מספר טלפון</th>
                    <th className="border font-weight-normal">אימות דו שלבי</th>
                    <th className="border font-weight-normal">תפקיד</th>
                </tr>
            </thead>
            ,
            <tbody key="2">
    ...
    const deleteUser = (user) => {
        ...
        props.user.getIdToken(true)
            .then(idToken => {
                props.deleteRequestWithToken(USER_URL, idToken, data)
                    .then(data => {
                        setUsers(data);
    ...
    const updateUserDetails = async (uid, fields) => {
        ...
        return await props.user.getIdToken(true)
            .then(async (idToken) => {
                return await props.patchRequestWithToken(USER_URL, idToken, data)
                    .then(result => {
    ...
    const updateUserPhoto = async (uid, photo) => {
        ...
        return await props.user.getIdToken(true)
            .then(async (idToken) => {
                return await props.postRequestWithToken(USER_PHOTO_URL, idToken, data)
                    .then(result => {
                        if (result.result === true) {
                            fetchUsers();
    ...
    return (
        <>
            ...
            />
            <div className="container-fluid my-4 mx-md-5 text-right"
                style={{ maxWidth: "95vw" }}>
                ...
                <div className="text-right mb-3">
                    <div className={"border d-inline-block rounded my-1 " + (searchFocus ? "userSearch" : "")} style={{ backgroundColor: "#f5f8fa" }}>
                        <FormControl
                ...
                <div className="d-flex justify-content-center">
                    <table className="text-break w-100 usersTable bg-white border"
